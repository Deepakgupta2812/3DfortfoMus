...
import { Cpu, Terminal, VolumeX, Volume2, Menu, X } from 'lucide-react'
    ...
    <nav className="fixed top-0 left-0 w-full z-50 px-4 md:px-6 py-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 group cursor-pointer">
          ...
          <span className="font-orbitron font-bold text-lg md:text-xl tracking-widest text-white">
        ...
        <div className="glass-panel px-6 py-2 rounded-full hidden md:flex gap-8 neon-border">
          {navLinks.map(l => (
            <button key={l.href} onClick={() => scrollTo(l.href)}
              className="hover:text-cyber-cyan transition font-mono text-sm bg-transparent border-none cursor-pointer text-white">
              {l.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2 md:gap-4 items-center">
          ...
          <button onClick={() => setMobileOpen(o => !o)}
            ...
            {mobileOpen ? <X className="w-5 h-5 text-cyber-cyan" /> : <Menu className="w-5 h-5 text-white" />}
          </button>
        </div>
      </div>
      ...
      {mobileOpen && (
        <div className="md:hidden mt-4 glass-panel rounded-xl p-4 flex flex-col gap-3 neon-border">
          {navLinks.map(l => (
            <button key={l.href} onClick={() => scrollTo(l.href)}
              className="text-left hover:text-cyber-cyan transition font-mono text-sm bg-transparent border-none cursor-pointer text-white py-2 border-b border-white/5">
              {l.label}
            </button>
          ))}
        </div>
